...
class TransactionHistory:
    ...
    def _parse_eth_transactions(
        ...
        for tx in raw_txs:
            try:
                ...
                usd_value = value_eth * eth_price if eth_price else 0
    ...
    def _parse_token_transactions(
        ...
        for tx in raw_txs:
            try:
                ...
                usd_value = amount * token_price if token_price else 0
    ...
    def get_transaction_summary(self, address: str) -> Dict:
        ...
        try:
            ...
            for tx in transactions:
                if tx["type"] == "Send":
                    total_sent += tx["usd_value"]
                else:
                    total_received += tx["usd_value"]

                # Calculate gas cost in USD (approximate)
                gas_cost_eth = (tx["gas_used"] * tx["gas_price"]) / 10**9
                eth_price = self.price_fetcher.get_token_price("ETH")
                if eth_price:
                    total_gas_spent += gas_cost_eth * eth_price
    ...
    def discover_user_tokens(self, address: str) -> Dict[str, str]:
        ...
        try:
            # Get recent token transactions to find token addresses
            token_txs = self._get_token_transactions(address, 50)

            discovered = {}
            for tx in token_txs:
                token_symbol = tx.get("token", "Unknown")
                # Extract token address from the transaction data
